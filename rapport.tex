...
\usetikzlibrary{matrix,graphs,graphs.standard}
...
\textit{Cette partie est inspirée voire recopiée du MacLane\cite{catswm}, car elle est utile pour la compréhension du rapport mais aussi car j'ai passé du temps dessus avant de passer à $\Theta$.}
...
TODO : recopier la preuve depuis le Git
...
\begin{itemize}
    ...
    \item Pour un morphisme générateur $y$, son interprétation sera notée $\Brackets{y}$
\end{itemize}
...
\[\
\Brackets{x^{(n_1,\dots,n_m)}_{i,j}} : (k_1,\dots,k_m) \mapsto (k_1,\dots,\Brackets{x^{n_i}_j}(k_i),\dots,k_m)
\]
...
\begin{itemize}
    \item Si $x=\delta$.\[
\Brackets{x^{(n_1,\dots,n_m)}_i} : (n_1,\dots,n_m)\mapsto (n_1,\dots,\hat{n_i},\dots,n_m)
\]
\item Si $x=\sigma$. \[
\Brackets{x^{(n_1,\dots,n_m)}_i} : (n_1,\dots,n_m)\mapsto (n_1,\dots,n_i,n_i,n_{i+2},\dots,n_m)
\]
\end{itemize}
...
Les deux premiers points correspondent à une zone de \textbf{commutation}.

Le suivant correspond à une zone de \textbf{disparation}.

Les trois derniers correspondent à une zone de \textbf{décalage}.
...
Le suivant correspond à une zone de \textbf{dédoublement}.

Les deux derniers correspondent à une zone de \textbf{décalage}.
...
\textdbend Pour assurer l'unicité, il faut néanmoins faire attention et imposer un autre sur la commutation des internes loins. Par exemple, l'ordre croissant sur le premier indice (sélection de la case)

La mise sous forme canonique des deux «côtés» est ensuite directe par la présentation de $\Delta$.

\subsubsection{Présentation}

Pour montrer la bijection de l'interprétation, il suffit de montrer que la fonction d'interprétation est bijective sur les formes canoniques.
En l'occurrence, on peut se restreindre à l'identité.

Or, la forme canonique de l'identité est nécessairement l'identité (c'est-à-dire le mot vide), et on a bien $\Brackets{\varepsilon} = \varepsilon$. Ainsi, on a bien une présentation par générateurs $(V_{n+1}, E_{n+1})$ et relations $\mathcal R_{n+1}$ de $\Theta_{n+1}$.
